...
export default function PayAsStudentPage() {
  ...
  useEffect(() => {
    ...
    modulesParam.split(',').filter(Boolean).forEach((entry) => {
      ...
      quantityMap[mod] = Math.max(0, Math.min(10, parseInt(qtyStr ?? '1', 10) || 1));
    });
  ...
  const handlePayNow = () => {
    ...
    router.push(`/pay-as-student/checkout?${params.toString()}`);
  };
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div>
        <h1 className="text-xl font-bold text-gray-900">Select your Modules</h1>
        <p className="mt-1 text-sm text-gray-500">
          Set the quantity for each module you want. Setting quantity to 1 or more adds it to your order.
        </p>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-neutral-200 bg-white">
        ...
        {/* Table rows */}
        {rows.map((row, idx) => {
          const isActive = activeModules.includes(row.value);
      ...
      <div className="rounded-xl border border-neutral-200 bg-white p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-600">
            Subtotal{totalUnits > 0 ? ` (${totalUnits} unit${totalUnits !== 1 ? 's' : ''})` : ''}
          </span>
